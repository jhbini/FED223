...
$(() => {
    ...
    /************************************************************* 
     ...
     1. 오른쪽 버튼 클릭시 배너는 오른쪽에서 왼쪽방향으로 
        ...
        3. 모든 배너는 무한이동을 원칙으로 함
        
        4. 배너이동시 배너의 순번을 블릿으로 표시함
        
        5. 자동넘김이 셋팅되어 있으며 사용자가 조작시
        자동넘김이 멈춰지고 일정시간 놔두면 다시 자동넘김 작동 
    ...
    $(".abtn").click(function () {
        ...
        if (isR) { // 오른쪽버튼
            ...
            // this가 헷갈리면 본인 이름 그대로 쓰기
            // 맨뒤요소를 맨앞에 이동
            slide.
            prepend(slide.find("li").last())
                // prepend(요소)자식요소로 앞에 추가 또는 이동
                // find(요소) 자손요소찾기
                // last() 마지막요소
        ...
        // 같은 순번 슬라이드는 오른쪽일 때 2번째 슬라이드(순번1)
        // 왼쪽일때 1번째 슬라이드(순번0)
        // eq(순번) -> ~중에 몇번째 요소
        // eq(isR?1:0) -> isR?1:0 -> 3항연산자
        // isR이 true이면(1이면) 1을출력, 아니면 0출력
        let sseq =
        ...
        // 해당순번(sseq)의 블릿 li에 클래스"on" 넣기
        indic.eq(sseq).addClass("on")
            // 다른 형제요소들 -> siblings() 은 클래스 지워
            //siblings는 형제들 중에 특별한 것들만 선택도 가능
    ...
    *********************************************************/
    slide.on("dragstop", function () {
        ...
        // offset은 절대적인 위치값을 잡기에는 최고지만 상대적 위치는 no
        // offset().left는 보이는 화면 왼쪽선 기준
        // position().left는 싸고있는 포지션있는 부모박스 기준함

        console.log("슬위:", spos);

        // 3. 위치값에 따라 슬라이드 이동하기
        // 기준값의 90%는 setval*0.9
        // 기준값의 110%는 setval*1.1
