...
$(() => {
    ...
    $(".abtn").click(function (e) { // e - 이벤트 전달 변수

        // a기본이동 막기 
        // (맨뒤 return false와 효과 동일)
        e.preventDefault();
        ...
        // 2. 버튼별 분기하여 기능구현
        if (isR) { // 오른쪽버튼
            // 클린된 버튼의 부모아래 상대적인 요소선택
            // console.log("난 누구?",$(this).siblings(".viewer>ul"));
            ...
            $(this).parent().find(".viewer>ul")
                .prepend(
                    ...
                    $(this).parent().find(".viewer>ul").find("li").last())
        ...
        $(this).parent().find(".indic>li").eq(sseq).addClass("on")
    ...
    indic.click(function (e) {
        ...
        if (diff > 0) { // 양수는 오른쪽에서 들어옴
            ...
            if (absd === indic.length - 1) {
                ...
                // 맨뒤로 첫번째 슬라이드 미리이동
                $(this).parents(".slider").find(".viewer>ul").append($(this).parents(".slider").find(".viewer>ul").find("li").first())
                    .css({
                        left: "100%"
                    });
                ...
                function () { // 이동후 실행함수
                    ...
                    for (let i = 0; i < absd; i++) {
                        ...
                        $(this).parents(".slider").find(".viewer>ul") // slide
                            .append($(this).parents(".slider").find(".viewer>ul>li").first())
                            .css({
                                left: (-100 * temp) + "%"
                            });
            ...
            if (absd === indic.length - 1) {
                ...
                $(this).parents(".slider").find(".viewer>ul").animate({
                    ...
                    () => { // 이동후 맨뒤요소 맨앞이동 left:0
                        $(this).parents(".slider").find(".viewer>ul").prepend($(this).parents(".slider").find(".viewer>ul").find("li").last())
                            .css({
                                left: "0"
                            });
                    } //// 콜백함수 ///
